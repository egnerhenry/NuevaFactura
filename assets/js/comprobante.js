//CALCULOS TOTALES Y SUBTOTALES (Metodos Prototipos)        
   ...
   comprobante.prototype.calcular = function(parent){
            ...
            if(tipoIgv < 8){
                ...
                console.log($("#anticipo").val());
                if($("#anticipo").val() == '0')
                {                
                    if (cmp.incluyeIgv == true ) {                        
                        this.subtotal = parseFloat((importe/(1+parseFloat(igv)))*cantidad-desc).toFixed(2);
                        this.subtotalCosto = parseFloat((importeCosto/(1+parseFloat(igv)))*cantidad).toFixed(2);

                        this.total = parseFloat(importe*cantidad).toFixed(1); 
                        this.totalVenta = this.total;
                        this.totalCosto = parseFloat(importeCosto*cantidad).toFixed(2); 
                        
                    } else {                                                
                        this.subtotal = parseFloat((importe*cantidad)-desc).toFixed(2);
                        this.subtotalCosto = parseFloat(importeCosto*cantidad).toFixed(2);

                        this.total = parseFloat(importe*cantidad).toFixed(1);                        
                        this.totalVenta = parseFloat(this.total*(1+parseFloat(igv))).toFixed(2);
                        this.totalCosto = parseFloat(importeCosto*cantidad).toFixed(2);
                    }
                    this.igv = parseFloat(this.totalVenta-this.subtotal).toFixed(2);  
                    this.igvCosto = parseFloat(this.totalCosto-this.subtotalCosto).toFixed(2);
                ...
                {                           
                    ...
                    this.totalVenta = this.total;
                    this.totalCosto  = parseFloat(this.subtotal).toFixed(2);

                    var baseImponible = this.total /(1+parseFloat(igv));
                    var baseImponibleCosto = this.totalCosto /(1+parseFloat(igv));
                    this.igv = parseFloat(this.total-baseImponible).toFixed(2);                     
                    this.igvCosto = parseFloat(this.total-baseImponible).toFixed(2);
                }
           ...
           $(parent).children().children('#totalVenta').val(this.totalVenta);
           $(parent).children().children('#totalCosto').val(this.totalCosto);
           $(parent).children().children('#igv').val(this.igv);        
           //$(parent).children().children('#igvCosto').val(this.igv);
           calcular();
        };
    ...
    function calcular(){        
        ...
        if(typeof(tabla.length) !== 'undefined'){
            $.each(tabla,function(indice,value){             
                ...
                var baseImponible=0;
                if(tipoIgv < 8){                   
                    //Tipo Igv Gratuito 21-07-2020 ---> Alexander Fernández//                   
                    if(tipoIgv < 2){                        
                        if($("#anticipo").val() == '0')
                        {
                            if(cmp.incluyeIgv == true)
                            {
                                baseImponible = itemTotal/(1+parseFloat(igv));                                
                                itemTotalConIgv = itemTotal;
                                itemIgv = itemTotalConIgv-baseImponible;
                            } else {                                                        
                                baseImponible = itemTotal;                                
                                itemTotalConIgv = baseImponible*(1+parseFloat(igv));
                                itemIgv = itemTotalConIgv-baseImponible;
                            }                                                                                                            
                            ...
                            itemTotalConIgv = itemTotal;
                            baseImponible = itemTotalConIgv/(1+parseFloat(igv));
                            itemIgv =  itemTotalConIgv-baseImponible;
                    ...
                    //Tipo Igv Gratuito 21-07-2020 ---> Alexander Fernández//
                    itemTotalConIgv  = itemTotal;
                    baseImponible    = itemTotalConIgv;
                    itemIgv =  itemTotalConIgv-baseImponible;                    
                    total_gratuita  += parseFloat(baseImponible);
                    total_igv += parseFloat(itemIgv);
                    itemTotalConIgv = 0;
                  }  
                }
            ...
            if($.isNumeric($('#descuento_global').val()))
            {
                ...
                if (_descuentoTotal > 0) {
                    ...
                    total_gravada = total_a_pagar /(1+parseFloat(igv));
            ...
            $('#total_gratuita').val(total_gratuita.toFixed(2));
